                ...
                <div class="eael-post-list-featured-inner" style="background-image: url(' . wp_get_attachment_image_url(get_post_thumbnail_id(), 'full') . ')">
                    <div class="featured-content">';
                        ...
                        if ($settings['eael_post_list_featured_excerpt'] === 'yes') {
                            $html .= '<p>' . implode(" ", array_slice(explode(" ", strip_tags(strip_shortcodes(get_the_excerpt() ? get_the_excerpt() : get_the_content()))), 0, $settings['eael_post_list_featured_excerpt_length'])) . '...</p>';
                        }
                    $html .= '</div>
                </div>
            </div>';
                            ...
                            $html .= '<div class="eael-post-list-content">';
                                ...
                                if ($settings['eael_post_list_post_excerpt'] === 'yes') {
                                    if ($settings['eael_post_list_layout_type'] == 'default' || ($settings['eael_post_list_layout_type'] == 'advanced' && $iterator !== 8)) {
                                        $html .= '<p>' . implode(" ", array_slice(explode(" ", strip_tags(strip_shortcodes(get_the_excerpt() ? get_the_excerpt() : get_the_content()))), 0, $settings['eael_post_list_post_excerpt_length'])) . '...</p>';
                                    }
                                }

                                if ($iterator == 8 && $settings['eael_post_list_layout_type'] == 'advanced') {
                                    $html .= '<p>' . implode(" ", array_slice(explode(" ", strip_tags(strip_shortcodes(get_the_excerpt() ? get_the_excerpt() : get_the_content()))), 0, 36)) . '...</p>';
                                }
                                            ...
                                            $html .= '<div class="author-meta">
                                                ...
                                                <div class="author-info">
                                                    <h5>' . get_the_author_posts_link() . '</h5>
                                                        <a href="' . get_day_link(get_post_time('Y'), get_post_time('m'), get_post_time('j')) . '"><p>
                                                        ' . get_the_date('d.m.y') . '</p></a>
                                                    </div>
                                                </div>
                                            </div>';
                                        }
